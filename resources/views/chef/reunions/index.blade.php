...
@section('title', 'Mes PV')
@section('content')
<div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold dark:text-white">Mes Procès-Verbaux</h1>
    <button onclick="openModal('createReunionModal')" class="px-4 py-2 bg-chef text-white rounded-lg hover:opacity-90"><i class="fas fa-plus mr-2"></i>Nouveau PV</button>
</div>
@if (session('success'))
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td class="px-4 py-3 flex gap-1">
                    ...
                    <form method="POST" action="{{ route('chef.reunions.soumettre', $reunion) }}">@csrf @method('PUT')<button type="submit" class="p-2 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg" title="Soumettre"><i class="fas fa-paper-plane"></i></button></form>
                    @endif
                    <form method="POST" action="{{ route('chef.reunions.destroy', $reunion) }}" onsubmit="return confirm('Supprimer ?')">@csrf @method('DELETE')<button type="submit" class="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"><i class="fas fa-trash"></i></button></form>
                </td>
            </tr>
            @endforeach
        </tbody>
...
<div id="createReunionModal" class="fixed inset-0 bg-black/50 hidden z-50 flex items-center justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg mx-4">
        ...
        <form method="POST" action="{{ route('chef.reunions.store') }}">
            @csrf
            <div class="space-y-4">
                ...
                <div><label class="block text-sm font-medium mb-1 dark:text-gray-300">Points abordés</label><textarea name="contenu" rows="5" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 dark:text-white"></textarea></div>
            ...
            <div class="mt-4 flex justify-end gap-2">
                ...
                <button type="submit" class="px-4 py-2 bg-chef text-white rounded-lg hover:opacity-90">Créer le PV</button>
            </div>
        </form>
    </div>
</div>
